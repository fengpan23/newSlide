	...
	function(ComponentView, etch, ComponentCommands, CmdListFactory, TouchBridge, RightMenu, lang) {
		...
		return ComponentView.extend({
			...
			_opacityChanged: function() {
				...
				var rgb = new RegExp("rgb" + PERMISSIVE_MATCH3);
				var rgba = new RegExp("rgba" + PERMISSIVE_MATCH4);
				
				var match;
				var rgb;
				if((match = rgb.exec(bg))) {
					rgb = { r: match[1], g: match[2], b: match[3] };
		        }
				if((match = rgba.exec(bg))){
					rgb = { r: match[1], g: match[2], b: match[3], a: match[4] };
				}
				if(bg.indexOf('#') === 0){
